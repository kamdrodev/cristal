...
const getAllLists = async (req, res, next) => {
    try {
        let queryOptions = {
            ...
            secondLanguage: req.query.secondLanguage
...
const getFlashcard = async (req, res, next) => {
    try {
        ...
        await List.findOne({
            _id: req.params.listId,
            userId: req.user.id,
            "flashcards._id": req.params.flashcardId,
        });
...
const updateFlashcard = async (req, res, next) => {
    try {
        ...
        await List.findOneAndUpdate(
            ...
            {
                $set: {
                    "flashcards.$.firstLanguage": req.body.firstLanguage,
                    "flashcards.$.secondLanguage": req.body.secondLanguage,
                },
            },
        );
            ...
            await List.findOneAndUpdate(
                {
                    _id: req.params.listId,
                    userId: req.user.id,
                    "flashcards._id": req.body.result.flashcards[i].flashcardId,
                },
                {
                    $inc: {
                        "flashcards.$.statistics.correctAnswers":
              req.body.result.flashcards[i].statistics.correctAnswers,
                        "flashcards.$.statistics.incorrectAnswers":
              req.body.result.flashcards[i].statistics.incorrectAnswers,
                    },
                },
            );
        }
